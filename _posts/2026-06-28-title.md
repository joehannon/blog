...
where $\nu$ is the stoichiometric coefficient (negative for reactants and positive for products). In a CFD context, the left hand side would be the substantive derivative.  The average rate of the j-th reaction is given by:

$$r_{j}=k_{j}\overline{\prod_{i} C_{i}^{o_{ij}}}$$

where $o_{ij}$ is the order of species i in reaction j, that is the exponent in the rate law. 
