...
Applies to **pure cross-stream** reaction schemes: every reaction consumes at least one
species from each feed stream.  All reactants are therefore absent from one of the two
pure-feed endpoints ($f=0$ or $f=1$), so $c_{\max}=0$ at both endpoints and the
complete-reaction limit collapses to a single-kink tent function.
